...
import nexa
from nexa.accounts.deferred_revenue import get_deferred_booking_accounts
from nexa.accounts.doctype.invoice_discounting.invoice_discounting import (
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from nexa.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def apply_tax_withholding(self):
		from nexa.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def build_gl_map(self):
		...
		company_currency = nexa.get_company_currency(self.company)
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from nexa.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": nexa.get_default_cost_center(self.company),
						},
					)
...
def get_outstanding(args):
	...
	company_currency = nexa.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = nexa.get_company_currency(company)
	account_details = frappe.get_cached_value(
