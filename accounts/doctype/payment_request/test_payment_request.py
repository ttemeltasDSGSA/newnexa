...
from nexa.accounts.doctype.payment_entry.test_payment_entry import create_payment_terms_template
...
class TestPaymentRequest(IntegrationTestCase):
	def setUp(self):
		...
		send_email = patch(
			"nexa.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email",
			return_value=None,
		)
		...
		_get_payment_gateway_controller = patch(
			"nexa.accounts.doctype.payment_request.payment_request._get_payment_gateway_controller",
		)
	...
	def test_multiple_payment_entries_against_sales_order(self):
		...
		pr1 = make_payment_request(
			dt="Sales Order", dn=so.name, recipient_id="nabin@nexa.com", return_doc=1
		)
		pr1.grand_total = 200
		pr1.submit()
	...
	def test_conversion_on_foreign_currency_accounts(self):
		...
		pr = make_payment_request(dt=po_doc.doctype, dn=po_doc.name, recipient_id="nabin@nexa.com")
