...
from nexa.accounts.doctype.subscription.subscription import DateTimeLikeObject, process_all
...
class ProcessSubscription(Document):
	...
	def process_all_subscription(self):
		...
		for subscription in create_batch(subscriptions, 500):
			frappe.enqueue(
				method="nexa.accounts.doctype.subscription.subscription.process_all",
				queue="long",
