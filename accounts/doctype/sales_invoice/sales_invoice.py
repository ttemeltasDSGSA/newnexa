...
import nexa
from nexa.accounts.deferred_revenue import validate_service_stop_date
from nexa.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_program_details_with_points,
	validate_loyalty_points,
)
from nexa.accounts.doctype.pricing_rule.utils import (
...
from nexa.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from nexa.accounts.general_ledger import get_round_off_account_and_cost_center
from nexa.accounts.party import get_due_date, get_party_account, get_party_details
from nexa.accounts.utils import (
	cancel_exchange_gain_loss_journal,
	get_account_currency,
	update_voucher_outstanding,
)
from nexa.assets.doctype.asset.depreciation import (
	depreciate_asset,
...
from nexa.assets.doctype.asset_activity.asset_activity import add_asset_activity
from nexa.controllers.accounts_controller import validate_account_head
from nexa.controllers.selling_controller import SellingController
from nexa.projects.doctype.timesheet.timesheet import get_projectwise_timesheet_data
from nexa.setup.doctype.company.company import update_company_current_month_sales
from nexa.stock.doctype.delivery_note.delivery_note import update_billed_amount_based_on_so
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from nexa.accounts.doctype.payment_schedule.payment_schedule import PaymentSchedule
		from nexa.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from nexa.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from nexa.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
		from nexa.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
		from nexa.accounts.doctype.sales_invoice_timesheet.sales_invoice_timesheet import (
			SalesInvoiceTimesheet,
		)
		from nexa.accounts.doctype.sales_taxes_and_charges.sales_taxes_and_charges import (
			SalesTaxesandCharges,
		)
		from nexa.selling.doctype.sales_team.sales_team import SalesTeam
		from nexa.stock.doctype.packed_item.packed_item import PackedItem
	...
	def check_credit_limit(self):
		from nexa.selling.doctype.customer.customer import check_credit_limit
	...
	def set_pos_fields(self, for_validate=False):
		...
		from nexa.stock.get_item_details import (
			ItemDetailsCtx,
			get_pos_profile,
	...
	def update_packing_list(self):
		if cint(self.update_stock) == 1:
			from nexa.stock.doctype.packed_item.packed_item import make_packing_list
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from nexa.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = nexa.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
	...
	def get_gl_entries(self, warehouse_account=None):
		from nexa.accounts.general_ledger import merge_similar_entries
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if cint(self.update_stock) and nexa.is_perpetual_inventory_enabled(self.company):
	...
	def apply_loyalty_points(self):
		from nexa.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
...
def get_list_context(context=None):
	from nexa.controllers.website_list_for_contact import get_list_context
...
def get_loyalty_programs(customer):
	...
	from nexa.selling.doctype.customer.customer import get_loyalty_programs
...
def create_dunning(source_name, target_doc=None, ignore_permissions=False):
	...
	def postprocess_dunning(source, target):
		from nexa.accounts.doctype.dunning.dunning import get_dunning_letter_text
