...
$.extend(nexa.queries, {
	...
	item: function (filters) {
		var args = { query: "nexa.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "nexa.controllers.queries.bom" };
	},

	task: function () {
		return { query: "nexa.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "nexa.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "nexa.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
nexa.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
nexa.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = nexa.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "nexa.controllers.queries.warehouse_query" });
