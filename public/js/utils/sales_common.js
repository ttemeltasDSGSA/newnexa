...
frappe.provide("nexa.selling");

nexa.sales_common = {
	setup_selling_controller: function () {
		nexa.selling.SellingController = class SellingController extends nexa.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], nexa.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", nexa.queries.contact_query);
				me.frm.set_query("company_contact_person", nexa.queries.company_contact_query);
				me.frm.set_query("customer_address", nexa.queries.address_query);
				me.frm.set_query("shipping_address_name", nexa.queries.address_query);
				me.frm.set_query("dispatch_address_name", nexa.queries.dispatch_address_query);
				me.frm.set_query("company_address", nexa.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "nexa.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				nexa.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				nexa.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				nexa.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				nexa.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			project() {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "nexa.projects.doctype.project.project.get_cost_center_name",
