...
import nexa
from nexa.controllers.item_variant import (
...
from nexa.stock.doctype.item_default.item_default import ItemDefault
from nexa.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from nexa.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from nexa.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from nexa.stock.doctype.item_default.item_default import ItemDefault
		from nexa.stock.doctype.item_reorder.item_reorder import ItemReorder
		from nexa.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from nexa.stock.doctype.item_tax.item_tax import ItemTax
		from nexa.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from nexa.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": nexa.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_nexa_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from nexa.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"nexa.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def convert_nexa_to_barcodenumber(nexa_number, barcode):
	if nexa_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
	...
	if nexa_number in convert:
		return convert[nexa_number]
...
def get_asset_naming_series():
	from nexa.assets.doctype.asset.asset import get_asset_naming_series
