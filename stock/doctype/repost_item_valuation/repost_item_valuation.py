...
import nexa
from nexa.accounts.general_ledger import validate_accounting_period
from nexa.accounts.utils import get_future_stock_vouchers, repost_gle_for_stock_vouchers
from nexa.stock.stock_ledger import (
...
def repost_gl_entries(doc):
	if not cint(nexa.is_perpetual_inventory_enabled(doc.company)):
		return
...
def execute_repost_item_valuation():
	...
	if name := frappe.db.get_value(
		"Scheduled Job Type",
		{"method": "nexa.stock.doctype.repost_item_valuation.repost_item_valuation.repost_entries"},
		"name",
	):
