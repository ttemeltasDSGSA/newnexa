...
frappe.provide("nexa.stock");
frappe.provide("nexa.accounts.dimensions");

nexa.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");
...
frappe.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				...
				return {
					query: "nexa.controllers.queries.get_batch_no",
					filters: filters,
				};
			}
		});
		...
		if (!check_should_not_attach_bom_items(frm.doc.bom_no)) {
			nexa.accounts.dimensions.update_dimension(frm, frm.doctype);
		}
	},
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new nexa.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "nexa.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function (frm) {
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					nexa.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					frappe.model.open_mapped_doc({
						method: "nexa.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
		...
		if (frm.doc.docstatus === 0) {
			frm.add_custom_button(
				__("Purchase Invoice"),
				function () {
					nexa.utils.map_current_doc({
						method: "nexa.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = nexa.utils.map_current_doc({
						method: "nexa.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus === 0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "nexa.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	validate_purpose_consumption: function (frm) {
		frappe
			.call({
				method: "nexa.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
	...
	make_retention_stock_entry: function (frm) {
		frappe.call({
			method: "nexa.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args: {
	...
	toggle_display_account_head: function (frm) {
		var enabled = nexa.is_perpetual_inventory_enabled(frm.doc.company);
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			frappe.call({
				method: "nexa.stock.utils.get_incoming_rate",
				args: {
					args: args,
				},
				callback: function (r) {
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			frappe.call({
				method: "nexa.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			frappe.call({
				method: "nexa.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						frappe.throw(__("BOM does not contain any stock item"));
					} else {
						nexa.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function (frm) {
		nexa.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		nexa.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) nexa.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			nexa.utils.map_current_doc({
				method: "nexa.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
frappe.ui.form.on("Stock Entry Detail", {
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return frappe.call({
				method: "nexa.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						...
						if (
							...
							nexa.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		nexa.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		nexa.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		nexa.stock.select_batch_and_serial_no(frm, child);
	},
});
...
var validate_sample_quantity = function (frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: "nexa.stock.doctype.stock_entry.stock_entry.validate_sample_quantity",
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
nexa.stock.StockEntry = class StockEntry extends nexa.stock.StockController {
	setup() {
		var me = this;

		this.barcode_scanner = new nexa.utils.BarcodeScanner({
			frm: this.frm,
			warehouse_field: (doc) => {
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return nexa.queries.item({ is_stock_item: 1 });
		};
		...
		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (nexa.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		this.frm.set_query("supplier_address", nexa.queries.address_query);
	}
	...
	refresh() {
		var me = this;
		nexa.toggle_naming_series();
		this.toggle_related_fields(this.frm.doc);
		this.toggle_enable_bom();
		this.show_stock_ledger();
		this.set_fields_onload_for_line_item();
		nexa.utils.view_serial_batch_nos(this.frm);
		if (this.frm.doc.docstatus === 1 && nexa.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		nexa.hide_company(this.frm);
		nexa.utils.add_item(this.frm);
		nexa.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);
		nexa.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
	}
	...
	company() {
		if (this.frm.doc.company) {
			...
			nexa.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "nexa.accounts.utils.get_company_default",
			args: {
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						nexa.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return frappe.call({
			method: "nexa.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new nexa.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		nexa.setup_serial_or_batch_no();
	}
	...
	toggle_related_fields(doc) {
		...
		if (nexa.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		nexa.utils.get_party_details(this.frm, null, null, null);
	}
};

nexa.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/nexa/js/utils/serial_no_batch_selector.js";
	...
	frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			...
			new nexa.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
...
nexa.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM", bom_no).then((bom) => {
		...
		nexa.stock.bom = { name, items: {} };
		items.forEach((item) => {
			nexa.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (nexa.stock.bom && nexa.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new nexa.stock.StockEntry({ frm: cur_frm }));
