...
import nexa
from nexa.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_available_serial_nos
from nexa.stock.doctype.warehouse.warehouse import get_child_warehouses
from nexa.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
from nexa.stock.valuation import FIFOValuation, LIFOValuation
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	from nexa.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=nexa.get_company_currency(args.get("company")),
	...
	if reposting_pending and throw_error:
		...
		frappe.msgprint(
			...
			primary_action={
				...
				"client_action": "nexa.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str, ctx: dict | str | None = None) -> BarcodeScanResult:
	def set_cache(data: BarcodeScanResult):
		frappe.cache().set_value(f"nexa:barcode_scan:{search_value}", data, expires_in_sec=120)
		_update_item_info(data, ctx)

	def get_cache() -> BarcodeScanResult | None:
		data = frappe.cache().get_value(f"nexa:barcode_scan:{search_value}")
		if not data:
			return
